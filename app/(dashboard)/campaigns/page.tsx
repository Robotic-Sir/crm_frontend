...
import { BellRing, Inbox, LayoutList, MessageSquarePlus, Settings2, Sparkles } from "lucide-react"
...
import { WebsiteEvents } from "@/components/campaigns/WebsiteEvents"
import { useCampaigns } from "@/hooks/use-campaigns"
import { useTemplates } from "@/hooks/use-templates"
import { getInboundMessages, getWebsiteEvents, getWhatsAppIntegration } from "@/lib/api/whatsapp"
import { useAuthStore } from "@/lib/store/auth"

type Tab = "campaigns" | "create" | "templates" | "inbox" | "events" | "settings"
...
const tabs: { key: Tab; label: string; icon: typeof LayoutList }[] = [
  ...
  { key: "events", label: "Website leads", icon: BellRing },
...
export default function CampaignsPage() {
  ...
  const websiteEvents = useQuery({ queryKey: ["website-events"], queryFn: getWebsiteEvents, enabled: isAdmin && tab === "events", refetchInterval: 30_000 })
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "events" && (websiteEvents.isLoading ? <Loading /> : <WebsiteEvents events={websiteEvents.data ?? []} />)}
      {tab === "inbox" && (
